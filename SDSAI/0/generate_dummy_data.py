...
def gen_messy_data(basepath):

  seedinit()

  # Function to introduce missing values
  def introduce_missing_values(data, percent=0.05):
      num_missing = int(len(data) * percent)
      indices = random.sample(range(len(data)), num_missing)
      for index in indices:
          data.iloc[index, random.randint(0, data.shape[1] - 1)] = None  # Randomly set a column to None
      return data

  # Function to introduce inconsistent values
  def introduce_inconsistent_values(data, percent=0.05):
      num_inconsistent = int(len(data) * percent)
      for _ in range(num_inconsistent):
          row = random.randint(0, len(data) - 1)
          col = random.randint(0, data.shape[1] - 1)
          if data.columns[col] == 'Age':
              data.iloc[row, col] = 'invalid'  # Set an invalid age
          elif data.columns[col] == 'Gender':
              data.iloc[row, col] = random.choice(['M', 'F', 'male', 'female', 'unknown'])  # Introduce inconsistent gender
      return data

  # Function to introduce outliers
  def introduce_outliers(data, percent=0.05):
      num_outliers = int(len(data) * percent)
      for _ in range(num_outliers):
          row = random.randint(0, len(data) - 1)
          col = random.choice(['Height (cm)', 'Weight (kg)'])
          if col == 'Height (cm)':
              data.iloc[row][col] = random.randint(250, 300)  # Height outlier
          elif col == 'Weight (kg)':
              data.iloc[row][col] = random.uniform(200, 300)  # Weight outlier
      return data

  # Function to introduce inconsistently formatted values
  def introduce_inconsistent_formatting(data, percent=0.05):
      num_inconsistent_format = int(len(data) * percent)
      for _ in range(num_inconsistent_format):
          row = random.randint(0, len(data) - 1)
          col = random.choice(['Admission_Date', 'Discharge_Date'])
          if col == 'Admission_Date':
              data.iloc[row][col] = 'WrongFormat'  # Invalid date format
          elif col == 'Discharge_Date':
              data.iloc[row][col] = '30-12-2023'  # Day-Month-Year format instead of standard
      return data


  data = {
      "Patient_ID": range(1000, 1000 + num_rows),
      "Name": ["Patient " + str(i) for i in range(num_rows)],
      "Age": [random.randint(18, 90) for _ in range(num_rows)],
      "Gender": [random.choice(['M', 'F']) for _ in range(num_rows)],
      "Height (cm)": [random.randint(150, 200) for _ in range(num_rows)],
      "Weight (kg)": [random.uniform(50, 100) for _ in range(num_rows)],
      "Blood_Pressure": [f"{random.randint(110, 150)}/{random.randint(70, 100)}" for _ in range(num_rows)],
      "Diabetes_Status": [random.choice(['Yes', 'No']) for _ in range(num_rows)],
      "Admission_Date": pd.date_range(start='2023-01-01', periods=num_rows, freq='D').strftime('%Y-%m-%d').tolist(),
      "Discharge_Date": pd.date_range(start='2023-01-01', periods=num_rows, freq='D').shift(-1).strftime('%Y-%m-%d').tolist()
  }

  df = pd.DataFrame(data)

  # Introduce missing values, inconsistent values, outliers, and inconsistently formatted values
  df = introduce_missing_values(df)
  df = introduce_inconsistent_values(df)
  df = introduce_outliers(df)
  df = introduce_inconsistent_formatting(df)

  # Save
  filepath = os.path.join(basepath, "messy_data.csv")
  df.to_csv(filepath, index=False)
  logger.info(f"Saved messy data to: {filepath}")
  return filepath
...
def main():
  ...
  # Lecture 3
  fn_messy_data = gen_messy_data(path_l3)
